...
| `collect`   | Collect   | Jeu de cartes, 2 a 6 joueurs. Reunir **deux sets de trois** personnages du meme anime. A son tour, une action : piocher, echanger au marche, scanner une main, **voler** (on designe une carte de dos chez un adversaire ; elle se retourne, et l on rend une carte de la MEME CLASSE — ou deux a defaut) ou poser un set. Poser ne refait PAS la main : on repart avec ce qui reste, et la pioche ajoute une carte tant que la main n est pas pleine. Le marche est une FILE : a chaque fin de tour celle de gauche part SOUS le paquet (jamais melee au
